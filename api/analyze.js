...
* 30~36分：建國中學、北一女中、師大附中、中山女高、成功高中、松山高中
* 25~28分：板橋高中(約26.6分)、市立大同、政大附中、麗山高中
* 22~24分：海山高中(約24.6分)、永平高中、景美女中、西松高中
* 19~21分：中和高中(約20~22分)、北大高中(約21~22分)、新店高中、新莊高中、丹鳳高中
* 16~18分：樹林高中(約17~18分，育林在地優免與繁星首選)、泰山高中、光復高中
...
💡 **準備建議**：
* 1句最具效益的補強建議（若學生有填夢想校系，建議可針對該校系的弱點科目給予衝刺方向）
...
總分36分：A++:7, A+:6, A:5, B++:4, B+:3, B:2, C:1；作文6級:1, 5級:0.8, 4級:0.6, 3級:0.4, 不知道:不計分。`;
  ...
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`, {      method: 'POST',
    ...
    if (data.error) {
      ...
      return res.status(200).json({ result: `【系統提示】分析暫時中斷 (${data.error.message})，請再點擊一次。` });
    }
